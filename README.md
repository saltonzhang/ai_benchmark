...
如果 `mail4.space` 现有 80/443 已被别的站点占用，不需要让 Node 抢 80/443。让 Node 继续监听 `127.0.0.1:8088`，再用现有 Nginx/Caddy 反代过去。

如果这个域名根路径已经有站点，可以改用子域名 `https://lab.mail4.space/`，或另配一个子路径。子路径部署需要额外处理静态资源路径，优先推荐子域名。
